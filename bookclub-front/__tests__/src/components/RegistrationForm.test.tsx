...
describe("RegistrationForm", () => {
  ...
  describe("input attributes", () => {
    test("inputs have correct types", () => {
      ...
      expect(
        (screen.getByLabelText("Email address") as HTMLInputElement).type,
      ).toBe("email");
    ...
    test("password field has minLength and pattern constraints", () => {
      ...
      expect(passwordInput.pattern).toBe(
        "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,}$",
      );
    });
    ...
    test("password field has title attribute with validation guidance", () => {
      ...
      expect(passwordInput.title).toContain(
