...
voteRouter.post(
  "/",
  userExtractor,
  async (req: Request<unknown, unknown, VoteRequest>, res: Response) => {
    ...
    if (req.user) {
      try {
        ...
        if (
          proposeResult.book_id === null ||
          proposeResult.book_id === undefined
        ) {
          res
            .status(400)
            .json({ error: "Proposal is not associated with a book!" });
          return;
        }
        ...
        if (
          cycleResult.bookclub_id === null ||
          cycleResult.bookclub_id === undefined
        ) {
          res
            .status(400)
            .json({ error: "Cycle is not associated with a book club!" });
          return;
        }
